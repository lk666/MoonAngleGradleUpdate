...
public class OrderEndDeliverDetailActivity extends Activity implements OnClickListener {
    private String TAG = "OrderEndDeliverDetailActivity";
    private ListView listView;
    private OrderProductAdapter adapter;
    private List<ProductEndDeliverVo> lists;
    ...
    private void init() {
        lists = new ArrayList<ProductEndDeliverVo>();
    ...
    private void getData() {
        ...
        DeliveryApi.getOutDeliverDetail(token, orderCode, deliverOrderDetailHandler);
    }
    ...
    private void initTipText() {
       // txt_fh_bill.setText(getResources().getString(R.string.text_deliver_ticket_tip2));
        txt_fh_bill.setText("");
        txt_list_order_detail_name.setText(getResources().getString(R.string.text_deliver_sent_detail));
        txt_commenName_fhck.setText(getResources().getString(R.string.text_deliver_store));
        txt_commenName_fh_address.setText(getResources().getString(R.string.text_deliver_address));
        txt_commonName_deliver_date.setText(getResources().getString(R.string.text_deliver_date));
        txt_customerName_fh_bill.setText(getResources().getString(R.string.text_deliver_upload_ticket));
        txt_commonName_deliver_shipper.setText(getResources().getString(R.string.text_recriver_store));
        txt_commenName_fhf_address.setText(getResources().getString(R.string.text_recrive_address));
    ...
    private void initCustomActionBar() {
        new CommonActionBar(getActionBar(), new IActionBarListener() {
            ...
            public void setTitle(TextView v) {
                ...
                v.setText(getResources().getString(R.string.detail_order_deliver_end));
            }
            @Override
    ...
    AsyncHttpResponseHandler deliverOrderDetailHandler = new TextHttpResponseHandler(
        ...
        public void onSuccess(int statusCode, Header[] headers,
            ...
            if (progressDialog != null)
                progressDialog.dismiss();
            try {
                ResultEndDeliverOrderBean deliverOrderDetailInfo = JSON.parseObject(responseString,
                        ResultEndDeliverOrderBean.class);
                if (deliverOrderDetailInfo.getResponseCode() == Constants.RESPONSE_RESULT_SUCCESS) {
                    getTicketPic();
                    setOutData(deliverOrderDetailInfo);
                }
            } catch (Exception e) {
                LogUtils.e(TAG, e.getMessage());
                PublicUtil.showToastServerBusy();
    ...
    private void setOutData(ResultEndDeliverOrderBean result) {
        if (result == null || result.getOutOrderDetail() == null || result.getOutOrderDetail().getProductDetails().size() < 1) {
            PublicUtil.showToastErrorData(main);
            return;
        }
        setOutHeadViewData(result);
        List<ProductEndDeliverVo> lis = result.getOutOrderDetail().getProductDetails();
        lists.addAll(lis);
    ...
    private void setOutHeadViewData(ResultEndDeliverOrderBean result) {
        ...
        ll_diff_layout.setVisibility(View.GONE);

        txt_source.setText(DateUtil.getTime(info.getOrderDate(), "yyyy-MM-dd"));
     ...
     /*   txt_fh_store.setText(info.getDeliStoreCode() + "-"
                + info.getDeliStoreType() + "-" + info.getDeliStoreChargeName());*/
        txt_fh_address.setText(info.getDeliStoreAddrName());
        txt_fh_phone.setText(info.getReceiveTel());
        txt_fh_phone.getPaint().setFlags(Paint.UNDERLINE_TEXT_FLAG);
        txt_fh_phone.getPaint().setAntiAlias(true);

        txt_fh_name.setText(info.getReceiveName());
        ...
        txt_fhf_address.setText(info.getReceiveAddr());
        ...
        String message = "<font color='#333333'>" + getString(R.string.txt_order_bottom_remarks) + "</font>" + "<font color='#666666'>" + ba + "</font>";
        txt_outBackup.setText(Html.fromHtml(message));
    }
    ...
    class OrderProductAdapter extends BaseAdapter {
        ...
        private List<ProductEndDeliverVo> list;
        Context context;

        public OrderProductAdapter(Context context, List<ProductEndDeliverVo> data) {
        ...
        public View getView(final int position, View convertView, ViewGroup parent) {
            ...
            holder.txt_title_amount.setText(R.string.order_title_delive_amount);
            holder.txt_boxes_counts.setText((String.format(getString(R.string.order_boxes_count),
                    ...
                    String.format(getString(R.string.order_product_count), list.get(position).getOutNum())));
